%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This script will replicate much of the analysis of the bid-ask spread 
% data of stock A on May 4, 2020, presented in Section 6 of the paper. 
%
...
% forecast_IVT.m
%
% Furthermore, the function simulate_IVT.m will be used in the 
% simulation/bootstrapping procedures.
%
% Note: There will be discrepancies in the standard errors and
% information criteria values, compared with those in the paper, due to the
% simulation/bootstrapping approach to calculating standard errors.(See 
% Appendix B.1 in the paper for further the details on the procedure).
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
clc; clear; close all;
cd(fileparts(which('analyze_stock_A.m')));
...
% Note: Set N = B = 500 to obtain similar results as in paper (note: SEs will be slightly different due to simulation/bootstrapping procedure).

%% Misc settings
lag = 30;    % For plotting ACF (max lag).
fh = 20;     % For forecasting (max horizon).
xMax = 60;   % For forecasting (max value for prediction).
n_oos = 720; % Number of observations in out-of-sample forecasting exercise.
...
%% Load data
load([quote_str,'_',date_str,'_data.mat']);
sec_incr = 5; % These data are sampled every 5 seconds.
dt = sec_incr/60;
n = length(y);

disp('----------------------------------------------------------------------');
disp(' ');
disp('Analyzing bid-ask spread data of stock A on May 4, 2020.');
disp(' ');
...
disp(' ');
disp(['Forecasting ',num2str(fh),' periods ahead using Poisson-Exp and NB-GAM models. In-sample data are y(1:',num2str(n_is),') and y(',num2str(n_is),') = ',num2str(y(n_is)),'...']);
tic
condForecastProb_IVT_PExp  = forecast_IVT(y(1:n_is),fh,dt,1,xMax,[],K);
condForecastProb_IVT_NBGAM = forecast_IVT(y(1:n_is),fh,dt,6,xMax,[],K);
time_forecast = toc;
disp(' ');
...
%% Print to screen
disp(' ');
disp('Information criteria (Table 2):');
...
disp('Estimation results (Table 3):');
disp(estRes);
...
datetick('x','HH:MM','keeplimits');%,'keepticks','keeplimits');
ylabel('Spread level','Interpreter','latex','FontSize',10);
grid on
title(['Bid-ask spread of ',quote_str,' on May 4, 2020'],'Interpreter','latex','FontSize',10);
...
for i = 1:4
    subplot(2,2,i);
    bar(0:xMax,condForecastProb_IVT_NBGAM(hvec(i),:));
    title(['NB-GAM ($h = ',num2str(hvec(i)),'$)'],'Interpreter','latex','FontSize',10);
    xlabel('$y(T+h)$','Interpreter','latex','FontSize',10);
    ylabel(['Predictive probability conditional on $y(T) = ',num2str(y(n_is)),'$'],'Interpreter','latex','FontSize',10);
end
