...
void c_pair_NB_Exp_v10(double *Y, double *par, double *hVec, int H, int N, double dt, double *fts)
{
 	mwIndex i,j,ih; 
    int c, hh;
    double A1, A2, temp, lambda, m, p, intens, Yt, Ys;
    ...
    for (ih=0; ih<H; ih++){
        ...
        A1 = m*Acom(hh*dt,lambda);
        A2 = m*Aunc(hh*dt,lambda);
        for (i=0; i<(N-hh); i++) {
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    c_pair_NB_Exp_v10(Y,par,hVec,H,N,dt,fts);
}
