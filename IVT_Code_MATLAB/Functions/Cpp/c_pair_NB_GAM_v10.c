...
void c_pair_NB_GAM_v10(double *Y, double *par, double *hVec, int H, int N, double dt, double *fts)
{
 	mwIndex i,j,ih; 
    int c, hh;
    double A1, A2, temp, HH, alpha, m, p, Yt, Ys, SQ;
    ...
    for (ih=0; ih<H; ih++){
        ...
        A1 = m*Acom(hh*dt,HH,alpha);
        A2 = m*Aunc(hh*dt,HH,alpha);
        for (i=0; i<(N-hh); i++) {
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    c_pair_NB_GAM_v10(Y,par,hVec,H,N,dt,fts);
}
