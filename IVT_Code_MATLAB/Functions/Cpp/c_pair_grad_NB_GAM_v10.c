...
void c_pair_grad_NB_GAM_v10(double *Y, double *par, double *hVec, int h, int N, double dt, int maxY, double *running_sum_unc, double *running_sum_com, double *fts)
{
 	mwIndex i,j,ih; 
    int c, hh;
    double A1, A2, Leb_com, Leb_unc, dH_Leb_com, da_Leb_com, dH_Leb_unc, da_Leb_unc, temp, temp_dm, temp_dp, temp_dH, temp_da, HH, alpha, m, p, Yt, Ys, P1, P2, P3;
    ...
    for (ih=0; ih<h; ih++){
        ...
        for (i=0; i<(N-hh); i++) {
            ...
            for (j=0; j<c+1; j++){
                ...
                temp_dH  = temp_dH + running_sum_unc[(int) ((Yt-j)+ih*(maxY+1))]*dH_Leb_unc*m*P1*P2*P3 + running_sum_unc[(int) ((Ys-j)+ih*(maxY+1))]*dH_Leb_unc*m*P1*P2*P3 + running_sum_com[(int) (j+ih*(maxY+1))]*dH_Leb_com*m*P1*P2*P3;
                temp_da  = temp_da + running_sum_unc[(int) ((Yt-j)+ih*(maxY+1))]*da_Leb_unc*m*P1*P2*P3 + running_sum_unc[(int) ((Ys-j)+ih*(maxY+1))]*da_Leb_unc*m*P1*P2*P3 + running_sum_com[(int) (j+ih*(maxY+1))]*da_Leb_com*m*P1*P2*P3;
            }
            if (temp>0){
                fts[0] = fts[0] + log(temp); 
                fts[1] = fts[1] + temp_dm/temp; 
                fts[2] = fts[2] + temp_dp/temp; 
                fts[3] = fts[3] + temp_dH/temp; 
                fts[4] = fts[4] + temp_da/temp;
            }
        }
    }
}
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    c_pair_grad_NB_GAM_v10(Y,par,hVec,h,N,dt,maxY,running_sum_unc,running_sum_com,fts);
}
